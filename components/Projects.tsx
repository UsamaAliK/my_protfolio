...
export default function Projects() {
  ...
  return (
    <section id="work" className="py-16 sm:py-24 border-b border-[#E8E8E4]/60">
      <div className="max-w-5xl mx-auto px-4 sm:px-6">
        ...
        <div className="space-y-10">
          {projects.map((project) => (
            <div
              key={project.id}
              className={`group bg-white border border-[#E8E8E4] rounded-xl p-6 sm:p-8 hover:border-[#0D9488]/50 transition-all duration-200 shadow-2xs hover:shadow-sm ${
                project.featured ? "ring-1 ring-[#0D9488]/20 bg-linear-to-b from-white to-[#FBFBF9]" : ""
              }`}
            >
              <div className="flex flex-col lg:flex-row justify-between lg:items-start gap-6 mb-6">
                <div className="space-y-2 max-w-2xl">
                  ...
                  <div className="flex items-center gap-3">
                    <span className="text-xs font-mono font-bold text-[#6B7280] group-hover:text-[#0D9488] transition-colors">
                      {project.number}
                    </span>
                    <span className="text-[#D1D1CB]">•</span>
                    {project.isLive && (
                      <span className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full bg-[#CCFBF1]/50 border border-[#99F6E4] text-[10px] font-mono text-[#0F766E] font-medium">
                ...
                <div className="shrink-0">
                  <a
                    ...
                    className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-[#F4F4F0] hover:bg-[#111827] hover:text-white border border-[#E8E8E4] text-[#111827] text-xs font-semibold tracking-wide transition-all group/btn"
                  >
                    <span>{project.linkText}</span>
                    <ArrowUpRight className="w-3.5 h-3.5 text-[#6B7280] group-hover/btn:text-white group-hover/btn:translate-x-0.5 group-hover/btn:-translate-y-0.5 transition-all" />
                  </a>
                </div>
              </div>
              ...
              {/* Tech Tags */}
              <div className="pt-4 border-t border-[#E8E8E4]/60 flex flex-wrap gap-1.5 items-center">
                <span className="text-[11px] font-mono text-[#9CA3AF] mr-2">Tech Stack:</span>
                {project.tech.map((t) => (
                  <span
                    key={t}
                    className="px-2.5 py-0.5 rounded bg-[#FBFBF9] border border-[#E8E8E4] text-[11px] font-mono text-[#4B5563]"
                  >
                    {t}
                  </span>
                ))}
